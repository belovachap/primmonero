...
#include "db.h"
#include "checkqueue.h" // has come after db.h for some reason...
#include "init.h"
#include "net.h"
#include "prime.h"
#include "txdb.h"
    ...
    CBlockIndex *pindexSlow = NULL;
    {
        ...
        if (fTxIndex) {
            ...
            if (pblocktree->ReadTxIndex(hash, postx)) {
                CAutoFile file(OpenBlockFile(postx, true), SER_DISK, VERSION);
                CBlockHeader header;
                try {
                    file >> header;
    ...
    do {
        ...
        do {
            ...
            if (pindexTest->pprev == NULL || pindexTest->pnext != NULL) {
                std::reverse(vAttach.begin(), vAttach.end());
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    for (unsigned int i=0; i<vtx.size(); i++)
    {
        ...
        pos.nTxOffset += ::GetSerializeSize(tx, SER_DISK, VERSION);
    }
    ...
    if (pindex->GetUndoPos().IsNull() || (pindex->nStatus & BLOCK_VALID_MASK) < BLOCK_VALID_SCRIPTS)
    {
        if (pindex->GetUndoPos().IsNull()) {
            CDiskBlockPos pos;
            if (!FindUndoPos(state, pindex->nFile, pos, ::GetSerializeSize(blockundo, SER_DISK, VERSION) + 40))
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    BOOST_LOG_TRIVIAL(info) << "SetBestChain new best height: " << nBestHeight << " "
                            << "date: " << DateTimeStrFormat("%Y-%m-%d %H:%M:%S %Z", pindexBest->GetBlockTime());
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    try {
        unsigned int nBlockSize = ::GetSerializeSize(*this, SER_DISK, VERSION);
    ...
    // @chapman TODO: Only need to do this once we feel like the chain is mostly
    // synced. 
    // // Relay inventory
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        try {
            unsigned int nBlockSize = ::GetSerializeSize(block, SER_DISK, VERSION);
            CDiskBlockPos blockPos;
            CValidationState state;
...
bool LoadExternalBlockFile(FILE* fileIn, CDiskBlockPos *dbp)
{
    ...
    int nLoaded = 0;
    try {
        CBufferedFile blkdat(fileIn, 2*MAX_BLOCK_SIZE, MAX_BLOCK_SIZE+8, SER_DISK, VERSION);
        uint64 nStartByte = 0;
        if (dbp) {
...
void IncrementExtraNonce(CBlock* pblock, CBlockIndex* pindexPrev, unsigned int& nExtraNonce, bool fNoReset)
{
    ...
    pblock->vtx[0].vin[0].scriptSig = (CScript() << nHeight << CBigNum(nExtraNonce) << std::vector<unsigned char>((const unsigned char*)pszDedication, (const unsigned char*)pszDedication + strlen(pszDedication))) + COINBASE_FLAGS;
