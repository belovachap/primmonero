...
BOOST_AUTO_TEST_CASE(multisig_verify)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        keys += key[i];
        s = sign_multisig(a_and_b, keys, txTo[0], 0);
        BOOST_CHECK_MESSAGE(!VerifyScript(s, a_and_b, txTo[0], 0, flags, 0), boost::format("a&b 1: %d") % i);

        keys.clear();
        keys += key[1],key[i];
        s = sign_multisig(a_and_b, keys, txTo[0], 0);
        BOOST_CHECK_MESSAGE(!VerifyScript(s, a_and_b, txTo[0], 0, flags, 0), boost::format("a&b 2: %d") % i);
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (i == 0 || i == 1)
            BOOST_CHECK_MESSAGE(VerifyScript(s, a_or_b, txTo[1], 0, flags, 0), boost::format("a|b: %d") % i);
        else
            BOOST_CHECK_MESSAGE(!VerifyScript(s, a_or_b, txTo[1], 0, flags, 0), boost::format("a|b: %d") % i);
    }
    s.clear();
    ...
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
        {
            ...
            if (i < j && i < 3 && j < 3)
                BOOST_CHECK_MESSAGE(VerifyScript(s, escrow, txTo[2], 0, flags, 0), boost::format("escrow 1: %d %d") % i % j);
            else
                BOOST_CHECK_MESSAGE(!VerifyScript(s, escrow, txTo[2], 0, flags, 0), boost::format("escrow 2: %d %d") % i % j);
        }
}
...
BOOST_AUTO_TEST_CASE(multisig_Sign)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        BOOST_CHECK_MESSAGE(SignSignature(keystore, txFrom, txTo[i], 0), boost::format("SignSignature %d") % i);
    }
}
